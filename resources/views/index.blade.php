        ...
        <div class="max-w-6xl mx-auto">
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6 mb-12">
                @forelse($featuredProducts as $product)
                    <div class="product-card bg-white rounded-lg shadow-md overflow-hidden transition-all duration-300 hover:-translate-y-2 hover:shadow-xl fade-in-up" data-delay="{{ 200 + ($loop->index * 100) }}">
                        <a href="{{ route('products.show', $product->slug) }}" class="block">
                            <div class="product-image aspect-[3/4] overflow-hidden bg-gray-100">
                                @if($product->cover_image)
                                    <img src="{{ asset('storage/' . $product->cover_image) }}" alt="{{ $product->title }}" class="w-full h-full object-cover transition duration-500 hover:scale-105">
                                @elseif($product->images->first())
                                    <img src="{{ asset('storage/' . $product->images->first()->image_path) }}" alt="{{ $product->title }}" class="w-full h-full object-cover transition duration-500 hover:scale-105">
                                @else
                                ...
                                @if($product->discount_price)
                                    <div class="absolute top-2 right-2 bg-red-500 text-white px-2 py-1 rounded text-xs font-bold">
                                        DISKON {{ round((1 - $product->discount_price / $product->price) * 100) }}%
                                    </div>
                                @endif
                            </div>
                        </a>
                        ...
                        <div class="p-4">
                            ...
                            @if($product->author)
                                <p class="text-sm text-gray-600 mb-2 line-clamp-1">{{ $product->author }}</p>
                            @endif
            ...
            <div class="text-center mt-12 fade-in-up" data-delay="1000">
                <a href="{{ route('products.index') }}" class="inline-block bg-blue-600 text-white px-8 py-3 rounded-lg text-lg font-semibold hover:bg-blue-900 transition">
                    Lihat Semua Produk →
                </a>
            </div>
        </div>

// Cek kondisi di awal file, sebelum melakukan apapun
if (window.location.pathname.includes('/create') || window.location.pathname.includes('/edit')) {
    console.log('Skipping entire product-show.js on create/edit page');
} else {
    document.addEventListener('DOMContentLoaded', function() {
        // Immediately exit if we're on the create page
        if (window.location.pathname.includes('/create')) {
            console.log('On create page - skipping product-show.js logic');
            return;
        }
        ...
        // Only proceed if we're on a product details page
        if (!productContainer || !productIdElement) {
            console.log('Not on a product details page, skipping product load');
            return;
        }

        const productId = productIdElement.value;

        // Only try to load a product if we have a valid product ID
        if (!productId || isNaN(parseInt(productId))) {
            console.log('Invalid product ID, skipping product load');
            return;
        }

        // Now we can safely load the product
        ...
        function loadProduct(id) {
            ...
            ApiService.products.getById(id)
                ...
                .catch(error => {
                    showError('An error occurred while loading product details: ' + (error.message || ''));
                    console.error(error);
                });
        }
